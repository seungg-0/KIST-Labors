...
for file_name in file_list:
    df = pd.read_csv(temp + file_name, encoding='euc-kr', usecols=[0, 1], index_col=0)
...
index = 0
for file_name in file_list:
    df = pd.read_csv(temp + file_name, encoding='euc-kr', index_col=0)
    area_name = file_name.split(".")[0]
    df["지역"] = area_name
    df = df.rename(columns={"신고일": "확진자수"})
